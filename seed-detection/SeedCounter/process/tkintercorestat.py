...
def get_residual(labeledarea, all=False):  # for divideloop and combineloop
    ...
    if all == False:
        ...
        residual, area = lm_method.lm_method_fit(
            lenlist, widlist, data, oldcoef, oldintercept)
        ...
        residual, area, coef, intercept = lm_method.lm_method(
            lenlist, widlist, data, all)
        lenwid = list(residual)
        data = list(area)
        areahist = {}
        residualhist = {}
            ...
            if linecount < len(elesize):
                if len(linelist) % elesize[linecount] == 0:
                    ...
                    sortline(linelist, midx, midy, linecount,
                             labellist=labellist, elesize=elesize)
                    linelist[:] = []
                    # colcount=1
                    linecount += 1
                    print(linecount)
        ...
        for j in range(len(nodelist[i])):
            if nodelist[i][j].check == False and nodearea[i][j] != 0:
                ...
                while (len(offspringlist) > 0):
                    ...
                    for k in range(len(y)):
                        if curri + y[k] >= 0 and curri + y[k] < len(nodelist) and currj + x[k] < len(
                                nodelist[i]) and currj + x[k] >= 0:
                            if nodelist[curri + y[k]][currj + x[k]].check == False and nodearea[curri + y[k]][
                                currj + x[k]] != 0:
                                nodelist[curri + y[k]][currj + x[k]].check = True
                                # print('nodelist i+ym,j+xn='+str(i+y[m])+','+str(j+x[n]))
                                offspringlist.append(
                                    nodelist[curri + y[k]][currj + x[k]])
                labelnum += 1
    for i in range(len(nodearea)):
        for j in range(len(nodearea[0])):
...
def manualboundarywatershed(area, avgarea):
    ...
    local_maxi = peak_local_max(distance, indices=False, footprint=numpy.ones(
        (masklength, masklength)), labels=area)
...
def cornerdivide(area, greatareas):
    ...
    while len(greatareas) > 0:
        ...
        if topkey not in exceptions:
            ...
            newtempsubarea = numpy.where(tempsubarea != 1., 0, 1)
            antitempsubarea = numpy.where(
                (tempsubarea != 1.) & (tempsubarea != 0), tempsubarea, 0)
...
def manualdivide(area, greatareas):
    ...
    while len(greatareas) > 0:
        ...
        if topkey not in exceptions:
            ...
            antitempsubarea = numpy.where(
                (tempsubarea != 1.) & (tempsubarea != 0), subarea, 0)
            # antitempsubarea=antitempsubarea*topkey.astype(int)
            times = len(locs[0]) / meanpixel
            # roundthreshold=0.6
            # times=round(times-roundthreshold+0.5)
            ...
            labelunique, labcounts = numpy.unique(
                newsubarea, return_counts=True)
            labelunique = labelunique.tolist()
            ...
            if len(labelunique) > 2:
                ...
                add = 1
                while newlabel > 1:
                    newsubarea = numpy.where(
                        newsubarea == topkey * newlabel, maxlabel + add, newsubarea)
                    print('new label: ' + str(maxlabel + add))
                    newlabelcount = len(numpy.where(
                        newsubarea == maxlabel + add)[0].tolist())
                    ...
                    print('add ' + 'label: ' + str(maxlabel + add) +
                          ' count=' + str(newlabelcount))
                    newlabel = labelunique.pop(-1)
                    add += 1
...
def divideloop(area, misslabel, avgarea, layers, par):
    ...
    while len(sortedkeys) > 0:
        ...
        if topkey != 0 and hist[topkey] > uprange and topkey not in exceptions:
            ...
            newsubarea = boundarywatershed(
                newtempsubarea, 1, 'inner')  # ,windowsize)
    ...
    while misslabel <= 0:  # or gocombine==True:
        ...
        if i not in tinyareas and hist[i] < lowrange:
            ...
            while (len(poscombines) > 0 and stop == False):
                ...
                combunique, combcount = numpy.unique(
                    toplist, return_counts=True)
                ...
                while (len(toplist) > 0):
                    ...
                    if top != 0:
                        ...
                        if hist[i] + topcount > lowrange and hist[i] + topcount < uprange:
                            localarea = combinecrops(
                                localarea, subarea, i, top, ulx, uly, rlx, rly)
                            stop = True
    ...
    if len(unique) > 2:
        ...
        for uni in tempunique[1:]:
            ...
            if currcount > (meanpixel) or currcount < (meanpixel - 3 * std):
                boundaryarea = numpy.where(
                    boundaryarea == uni, 0, boundaryarea)
...
def findcoin(area):
    ...
    for key in topfive:
        ...
        newtempsubarea = numpy.where(tempsubarea != 1., 0, 1)
        newsubarea = boundarywatershedcoin(
            newtempsubarea, 1, 'inner')  # ,windowsize)
        labelunique, labcounts = numpy.unique(newsubarea, return_counts=True)
        hist = dict(zip(labelunique[1:], labcounts[1:]))
        labelunique = labelunique.tolist()
        ...
        if topkey not in exceptions:
            if hist[topkey] > maxthres or topkeylw > maxlw:
                ...
                antitempsubarea = numpy.where(
                    (tempsubarea != 1.) & (tempsubarea != 0), subarea, 0)
                newsubarea = boundarywatershed(
                    newtempsubarea, 1, 'inner', itertime=2)
                labelunique, labcounts = numpy.unique(
                    newsubarea, return_counts=True)
                labelunique = labelunique.tolist()
                if len(labelunique) > 2:
        ...
        if topkey not in exceptions:
            if hist[topkey] < minthres or topkeylw < minlw:
                ...
                for j in range(1, 11):
                    ...
                    if rly + j < area.shape[0] and stop == False and len(down_unique) < 2:
                        ...
                        for x in range(len(down_unique)):
                            if down_unique[x] > 0:
                                tempdict = {
                                    down_unique[x]: hist[down_unique[x]]}
                                maxlabel.update(tempdict)
                    if ulx - j >= 0 and stop == False and len(left_unique) < 2:
                        ...
                        for x in range(len(left_unique)):
                            if left_unique[x] > 0:
                                tempdict = {
                                    left_unique[x]: hist[left_unique[x]]}
                                maxlabel.update(tempdict)
                    if ulx + j < area.shape[1] and stop == False and len(right_unique) < 2:
                        ...
                        for x in range(len(right_unique)):
                            if right_unique[x] > 0:
                                tempdict = {
                                    right_unique[x]: hist[right_unique[x]]}
                                maxlabel.update(tempdict)
                    print(up_unique, down_unique, left_unique, right_unique)
                    tempcombines.append(up_unique)
                    tempcombines.append(down_unique)
                ...
                while (len(poscombines) > 0 and stop == False):
                    ...
                    toplist = dict(zip(combunique, combcount))
                    toplist = list(
                        sorted(toplist, key=toplist.get, reverse=True))
                    while (len(toplist) > 0):
                        top = toplist.pop(0)
                        if top != 0:
                            topcount = hist[top]
                            toplocs = numpy.where(area == top)
                            ulx, uly = min(topkeylocs[1].tolist(
                            ) + toplocs[1].tolist()), min(topkeylocs[0].tolist() + toplocs[0].tolist())
                            rlx, rly = max(topkeylocs[1].tolist(
                            ) + toplocs[1].tolist()), max(topkeylocs[0].tolist() + toplocs[0].tolist())
                            combinelength = rly - uly
                            combinewidth = rlx - ulx
                            combinediag = (combinelength ** 2 +
                                           combinewidth ** 2) ** 0.5
                            ...
                            if hist[topkey] + topcount > minthres and hist[topkey] + topcount < maxthres:
                                ...
                                if tempresidual > minlw and tempresidual < maxlw:
                                    area = combinecrops(
                                        area, subarea, topkey, top, ulx, uly, rlx, rly)
                                    stop = True
                                    # unique, counts = numpy.unique(area, return_counts=True)
                                    # unique=unique[1:]
                                    # counts=counts[1:]
...
def get_mapcolortable(area, inputelesize, inputlabellist):
    riginmethod, misslabel, localcolotable = relabel(
        area, elesize=inputelesize, labellist=inputlabellist)
    copycolortable = {**colortable}
    return copycolortable
...
def resegmentinput(inputlabels, minthres, maxthres, minlw, maxlw):
    ...
    areahist, residualhist, oldcoef, oldintercept = get_residual(
        inputlabels, True)
    validation, godivide, gocombine = resegvalidation(
        minthres, maxthres, hist, minlw, maxlw, inputlabels)
    lastgreatarea = []
    lasttinyarea = []
    ...
    while (validation == False):
        ...
        hist = dict(zip(unique, counts))
        validation, godivide, gocombine = resegvalidation(
            minthres, maxthres, hist, minlw, maxlw, labels)
    labels = renamelabels(labels)
    unique, counts = numpy.unique(labels, return_counts=True)
    # unique=unique[1:]
    # counts=counts[1:]
...
def processinput(input, validmap, avgarea, layers, ittimes=30, coin=True, shrink=0):
    ...
    allinexceptsions, godivide, gocombine = checkvalid(
        p, leftsigma, rightsigma)
    par = 0.0
...
def kmeansprocess(pixellocs, input, counts):
    ...
    clf = KMeans(n_clusters=counts, init='k-means++',
...
def init(input, validmap, map, layers, ittimes, coin):
    ...
    labels, res, colortable, labeldict = firstprocess(
        input, validmap, avgarea, occupiedratio)
    # else:
...
def pool_backward(dA, cache):
    ...
    for h in range(n_H):  # loop on the vertical axis
        for w in range(n_W):  # loop on the horizontal axis
            ...
            horiz_start:horiz_end] += restore_value(da, shape)
