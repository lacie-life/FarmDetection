...
def manualboundarywatershed(area, avgarea):
    ...
    while (len(unique) - 1 > possiblecount):
        maskpara += 0.1
        masklength = int((avgarea * maskpara) ** 0.5) - 1
        local_maxi = peak_local_max(distance, indices=False, footprint=numpy.ones((masklength, masklength)),
                                    labels=area)
        markers = ndi.label(local_maxi)[0]
        unique = numpy.unique(markers)
    while (len(unique) - 1 < possiblecount):
...
def init(input, validmap, map, layers, ittimes, coin):
    ...
    labels, res, colortable, greatareas, tinyareas, coinparts = processinput(input, validmap, avgarea, layers, ittimes,
                                                                             coin, shrink)
    # else:
