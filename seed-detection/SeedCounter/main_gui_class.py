...
from ttkbootstrap.dialogs import Messagebox, Querybox, Dialog
# camera_fps= 0.5
#
...
cwd = os.getcwd()
rice_batchfilepath = cwd + '/' + "test/config/test-batch.txt"
...
class batch_ser_func():
    ...
    def export_result(self):
        ...
        #export summary data
        # with open(self.exportpath + '/summary-outputdata.csv', 'w') as f:
        #     # create the csv writer
        #     writer = csv.writer(f)
        #
        #     # write a row to the csv file
        #
        #     header = ['num', 'weight', 'avg']
        #     row = [len(datatable),'' ,'' ]
        #     writer.writerow(header)
        #     writer.writerow(row)
        df = pd.DataFrame([[len(datatable), '', '']], columns=['Num', 'Weight', 'Avg'])
        df.to_csv(self.exportpath + '/summary-outputdata.csv', index= False)
...
def batch_process():
    ...
    if len(batch_filenames) == 0:
        print('No files', 'Please load images tImageTk.PhotoImage(Image.open("nocam.jpg")o process')
        return
...
class SeedCounter(ttk.Frame):
    def __init__(self, master):
        super().__init__(master)
        master.geometry = "1400x700"
        self.pack(fill=BOTH, expand=YES)

        self.image_noCam = ImageTk.PhotoImage(Image.open("nocam.jpg"))
        self.logo = ImageTk.PhotoImage(Image.open("logo.png").resize((100, 100), Image.ANTIALIAS))
        self.camera = camera.CanonCamera()
        self.input_image = None
        self.path_type = ttk.StringVar()
        """grid config"""
        ...
        # create view info Label frame
        view_info = ttk.Labelframe(col1, text='Camera View', padding=10, width=50, height=20)
        view_info.pack(side=TOP, fill=BOTH, expand=YES)

        # image view panel

        self.panel = ttk.Label(view_info)
        ...
        # control panel
        control_info = ttk.Labelframe(col2, text='Control Panel', padding=(15, 10))
        control_info.pack(side=TOP, fill=BOTH, expand=YES)

        # set type and weight
        type_info = ttk.Labelframe(
            master=control_info,
            text='Grains',
            padding=(15, 10)
        )
        ...
        ttk.Label(type_info, text="Select the grain: ").pack(side=LEFT)

        cbo = ttk.Combobox(
            master=type_info,
            values=['Rice', 'Peanut', 'Corn'],
            textvariable= self.path_type
        )
        ...
        process_button.pack(side=TOP, padx=5, pady=5)

        # self.process_prb = ttk.Progressbar(process_info,orient = 'horizontal', length = 200, mode = 'indeterminate')
        # self.process_prb.pack(side = TOP, padx = 5, pady = 5)

        # enter weight entry

        set_weight_button = ttk.Button(
            master=control_info,
            text="SET WEIGHT",
            command=self.on_click_set_weight,
            bootstyle="success-outline",
            width=10,
        )
        set_weight_button.pack(side=TOP, padx=5, pady=5)

        # get result

        get_result_button = ttk.Button(
            master=control_info,
            text="GET RESULTS",
            command=self.on_click_get_results,
            bootstyle="success",
            width=10,
        )
        get_result_button.pack(side=TOP, padx=5, pady=5)

        # licence info
        lic_info = ttk.Labelframe(col2, text='License Info', padding=20)
        ...
        lic_title = ttk.Label(
            master=lic_info,
            text="      Faculty of Agriculture Technology \n"
                 "University of Engineering and Technology"
            ,
            anchor=CENTER
        )
        lic_title.pack(fill=X, pady=(20, 20))

        # logo image
        ttk.Label(lic_info, image=self.logo).pack(side=TOP, padx=20, pady=20)
        # about button

        about_button = ttk.Button(
            master=lic_info,
            text="About Us!",
            command=self.on_clicked_about,
            bootstyle=INFO,
            width=10,
        )
        about_button.pack(side=TOP, padx=20, pady=20)

        # testPreview()
        self.testPreview()
    def on_clicked_about(self):
        Messagebox.show_info(title="About Us!", message="Created by FAT Lab Team!")

    def on_click_set_weight(self):
        self.weight = Querybox.get_float(prompt='Enter the weight (g): ', title='Set the weight', initialvalue=None,
                                         minvalue=0, maxvalue=100000)
        if self.weight is not None and self.weight > 0.0:
            Messagebox.show_info(title="Set the weight",
                                 message="You've successfully set the weight to " + str(self.weight) + "g")
        # filename = self.exportpath + '/' + originfile + '-outputdata.csv'
        # with open(filename, mode='w') as f:
        #     csvwriter = csv.writer(f)
        #     csvwriter.
        df = pd.read_csv('test/output/summary-outputdata.csv')

        df.loc[0, 'Weight'] = self.weight
        df.loc[0, 'Avg'] = self.weight/df.loc[0, 'Num']
        df.to_csv('test/output/summary-outputdata.csv', index = False)
    def on_click_get_results(self):
        outputfolder = cwd + '/' + "test/output"
        subprocess.Popen(["xdg-open", outputfolder])
    def testPreview(self):

        if (self.camera.isconnec):
            if self.camera.isplay:
                if not self.camera.ispreview:
                    self.camera.ispreview = True
                    try:
                        camera_file = gp.check_result(gp.gp_camera_capture_preview(self.camera.camera))
                        ...
                        print("Frame Read erroreeks.jpg")
                    self.camera.ispreview = False
            elif self.camera.is_capture_image:
                self.camera.is_capture_image = False
                # self.pixmap = self.PILimageToQImage(self.capture_image)
                # self.imageLabel.setPixmap(self.pixmap)
                image = Image.open("test/input/test.png")
                imageResize = image.resize((960, 640), Image.ANTIALIAS)
                imageResize = ImageTk.PhotoImage(imageResize)
                self.panel.configure(image=imageResize)
                self.panel.image = imageResize
                self.panel.pack(side="top", fill="both", expand="yes")
            elif self.camera.is_load_image:
                self.camera.is_load_image = False
                imageResize = self.camera.imageView.resize((960, 640), Image.ANTIALIAS)
                #
                # # Creating object of PhotoImage() class to display the frame
                imageDisplay = ImageTk.PhotoImage(imageResize)
                #
                # # Configuring the label to display the frame
                self.panel.config(image=imageDisplay)
                #
                # # Keeping a reference
                self.panel.photo = imageDisplay
                self.panel.pack(side="top", fill="both", expand="yes")
            ...
            self.panel.pack(side=TOP, expand=YES)
        self.panel.after(10, self.testPreview)

    def startCapture(self):
        self.pause_()
        self.camera.exit_camera()
        print(self.camera.isconnec)

        OK, path = self.camera.capture_image()

        if OK:
            self.camera.image_capture = Image.open(path)
            # self.camera.image_capture.show()
            self.camera.is_capture_image = True

        else:
            self.camera.showMesage("Capture Fail..", "..")
    ...
    def pause_(self):
        self.camera.isplay = False


    def play_(self):
        self.camera.isplay = True


    def process(self):


        type_of_grain = self.path_type.get()
        print(type_of_grain)

        if type_of_grain == 'Rice':
            path = rice_batchfilepath
        ...
        self.camera.output_image = Image.open("test/output/test-extwht-sizeresult.png")
        Messagebox.show_info(title="Process",
                             message="Processing is finished")
        self.camera.output_image.show()
    ...
    def loadImages(self):
        ...
        self.camera.imageView = Image.open(openDirectory)
        self.camera.is_load_image = True
        self.camera.imageView.save("test/input/test.png")
...
if __name__ == '__main__':
    app = ttk.Window("Seed Counter", "flatly", size=[1400, 760], resizable=[True, True])
