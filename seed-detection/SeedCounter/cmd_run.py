...
cwd = os.getcwd()
batchfilepath = cwd + '/' + "test/config/check-batch.txt"
...
class batch_ser_func():
    ...
    def Open_batchimage(self):
        try:
            ...
            self.batch_Multiimage.update({self.file: RGBfile})
    ...
    def oneband(self):
        ...
        self.RGB_vector = np.zeros((self.displayfea_l * self.displayfea_w, 3))
        ...
        pcabandsdisplay = pcabandsdisplay.reshape(self.displayfea_l, self.displayfea_w, featurechannel)
        tempdictdisplay = {'LabOstu': pcabandsdisplay}
        self.batch_displaybandarray.update({self.file: tempdictdisplay})
        self.baych_originbandarray.update({self.file: originbands})
    ...
    def singleband(self):
        ...
        indexbands = np.zeros((self.colorindex_vector.shape[0], 3))
    ...
    def colorindices_cal(self):
        ...
        print(self.displayfea_l, self.displayfea_w)
        colorindex_vector = np.zeros((self.displayfea_l * self.displayfea_w, 7))
        if channel == 1:
            Red = bands[0, :, :]
            Green = bands[0, :, :]
            Blue = bands[0, :, :]
        ...
        self.colorindicearray.update({self.file: originbands})
    ...
    def generateimgplant(self, displaylabels):
        ...
        if len(kmeans_sel) == 1:
            tempdisplayimg = np.where(displaylabels == kmeans_sel[0] - 1, 1, tempdisplayimg)
        else:
            for i in range(len(kmeans_sel)):
                sk = kmeans_sel[i] - 1
    ...
    def extraction(self, currentlabels):
        ...
        print("Nonzeroraito: ", nonzeroratio)
        ...
        if nonzeroratio <= 0.2:  # and nonzeroratio>=0.1:
            ...
            print("ratio to workingimg", ratio)
