...
from utils import axistest
from utils import lm_method
from utils import sel_area
...
def PCbuttons(frame, displayframe):
    ...
    for i in range(len(pcbuttons)):
        butimg = pcbuttons[i]
        but = Button(buttonframe, text='', image=butimg, compound=TOP,
                     command=partial(buttonpress, i, displayframe, buttonframe))
        if i == buttonvar.get():
...
def generatedisplayimg(filename):  # init display images
    ...
    try:
        ...
        resize = cv2.resize(Multiimage[filename], (int(resizeshape[0]), int(resizeshape[1])),
    ...
    try:
        tempdict.update({'Size': resize.shape})
        tempdict.update({'Image': ImageTk.PhotoImage(
            Image.fromarray(np.zeros((int(resizeshape[1]), int(resizeshape[0]))).astype('uint8')))})
    ...
    try:
        ...
        # colortempdict={}
        colordeviate = cv2.resize(colordeviate, (int(resizeshape[0]), int(resizeshape[1])),
                                  interpolation=cv2.INTER_LINEAR)
        ...
        # tempdict={}
        temppreview = cv2.resize(colordeviate, (int(previewshape[0]), int(previewshape[1])),
                                 interpolation=cv2.INTER_LINEAR)
...
def singleband_oldversion(file):
    ...
    displaybands = cv2.resize(bands, (int(bandsize[1] / ratio), int(bandsize[0] / ratio)),
                              interpolation=cv2.INTER_LINEAR)
    ...
    if 'NDI' not in originbands:
        ...
        displaybands = cv2.resize(NDI, (int(bandsize[1] / ratio), int(bandsize[0] / ratio)),
                                  interpolation=cv2.INTER_LINEAR)
    ...
    if 'Greenness' not in originbands:
        ...
        image = cv2.resize(Greenness, (int(bandsize[1] / ratio), int(bandsize[0] / ratio)),
                           interpolation=cv2.INTER_LINEAR)
...
def changeimage(frame, filename):
    ...
    if filename in multi_results.keys():
        ...
        itervar = IntVar()
        itervar.set(iternum)
        resscaler = Scale(resviewframe, from_=1, to=iternum, tickinterval=1, length=220, orient=HORIZONTAL,
                          variable=itervar, command=partial(changeoutputimg, filename))
        resscaler.pack()
...
def generatecheckbox(frame, classnum):
    ...
    for i in range(10):
        ...
        ch = Checkbutton(checkboxframe, text=dictkey, variable=checkboxdict[dictkey],
                         command=partial(changeclusterbox, ''))  # ,command=partial(changecluster,''))
        ...
        if len(path) > 0:
            ...
            for uni in uniquelabels:
                if uni != 0:
                    ...
                    tempdict.update({colortable[uni]: [kernellength, width, pixelmmratio ** 2 * len(pixelloc[0]),
    ...
    for file in files:
        ...
        if len(path) > 0:
            ...
            with open(filename, mode='w') as f:
                csvwriter = csv.writer(f)
                rowcontent = ['Index', 'Plot', 'Area(#pixel)', 'Length(#pixel)', 'Width(#pixel)', 'Area(mm2)',
                              'Length(mm)', 'Width(mm)']
                for key in indicekeys:
...
def gen_convband():
    ...
    if int(ratio) > 1:
        ...
        if forward == -1:
            print('pool_backward')
            cache = (np.zeros((processlabel.shape[0] * ratio, processlabel.shape[1] * ratio)),
                     {"f": int(ratio), "stride": int(ratio)})
...
def extraction():
    ...
    global batch
    if int(kmeans.get()) == 1:
        messagebox.showerror('Invalid Class #',
                             message='#Class = 1, try change it to 2 or more, and refresh Color-Index.')
        return
    refarea = None
    multi_results.clear()
    kernersizes.clear()
    ...
    if dealpixel < 512000:
        ...
        if nonzeroratio <= 0.2:  # and nonzeroratio>=0.1:
            ...
            workingimg = cv2.resize(currentlabels,
                                    ...
                                    interpolation=cv2.INTER_LINEAR)
            # if selarea.get()=='1':
            filter = cv2.resize(filter, (int(currentlabels.shape[1] / ratio), int(currentlabels.shape[0] / ratio)),
                                interpolation=cv2.INTER_LINEAR)
            workingimg = np.multiply(workingimg, filter)
            # else:
            #     #ratio=1
    ...
    if originlabels is None:
        originlabels, border, colortable, originlabeldict = tkintercorestat.init(workingimg, workingimg, '', workingimg,
                                                                                 10, coin)
        changekmeans = False
    else:
        if changekmeans == True:
...
def clustercontent(var):
    ...
    # if var=='1':
    cluster = ['LabOstu', 'NDI', 'Greenness', 'VEG', 'CIVE', 'MExG', 'NDVI', 'NGRDI', 'HEIGHT', 'Band1', 'Band2',
               'Band3']
    ...
    for key in cluster:
        ...
        ch = ttk.Checkbutton(contentframe, text=key, variable=bandchoice[
            key])  # ,command=changecluster)#,command=partial(autosetclassnumber,clusternumberentry,bandchoice))
...
pccombinebar_up = ttk.Scale(pcaframe, from_=0, to=1, length=350, orient=HORIZONTAL, variable=pc_combine_up,
                            command=changepcweight)  # ,command=partial(pcweightupdate,'',imageframe))#,command=partial(print,pc_combine_up.get))
pc_combine_up.set(0.5)
...
kmeansbar = ttk.Scale(kmeanslabel, from_=1, to=10, length=350, orient=HORIZONTAL, variable=kmeans, cursor='hand2',
                      command=partial(generatecheckbox, checkboxframe))
kmeansbar.pack()
...
# disbuttonoption={'Origin':'1','ColorIndices':'3','Output':'4'}
for (text, v1), (name, v2) in zip(disbuttonoption.items(), buttonname.items()):
    b = Radiobutton(buttondisplay, text=name, variable=displaybut_var, value=disbuttonoption[text],
                    command=partial(changedisplayimg, imageframe, text))
    b.pack(side=LEFT, padx=20, pady=5)
    b.configure(state=DISABLED)
    if disbuttonoption[text] == '1':
        b.invoke()
