...
cwd=os.getcwd()
batchfilepath=cwd+'/'+args[1]
print('batchfilepath',batchfilepath)
inputfolder=cwd+'/'+args[2]
print('inputfolder',inputfolder)
outputfolder=cwd+'/'+args[3]
print('outputfolder',outputfolder[3])
existfiles=[]
...
for file in localfiles:
    if file.find('.JPG')!=-1:
        ...
        if existfilename not in existfiles:
            existfiles.append(existfilename)

batch_colorbandtable=np.array([[255,0,0],
                               [255,127,0],
                               [255,255,0],
                               [127,255,0],
                               [0,255,255],
    ...
    if len(btfile)>0:
        if '.txt' in btfile:
            with open(btfile,mode='r') as f:
                ...
                print('Batch settings','PCweight='+str(pcweight)+'\nPCsel='+str(pcs)+'\nKMeans='+str(kmeans)+
